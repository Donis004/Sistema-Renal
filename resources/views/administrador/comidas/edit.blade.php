...
<head>
  <meta charset="UTF-8" />
  <title>Editar Comida - Sistema Renal</title>
      ...
      <div class="col-md-10">
        ...
        <div class="header-section">
          <div class="row align-items-center">
            <div class="col-md-8">
              <h2 class="mb-1"><i class="bi bi-pencil-square me-2"></i>Editar Comida</h2>
              <p class="mb-0 opacity-75">Modifica los datos de la comida</p>
            </div>
        ...
        <div class="card-custom">
          <form method="POST" action="{{ route('administrador.comidas.update', $comida->id_comida) }}">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <select class="form-select form-control-custom @error('id_paciente') is-invalid @enderror" 
                  ...
                  @isset($pacientes)
                    @foreach($pacientes as $paciente)
                      <option value="{{ $paciente->id_paciente }}" {{ old('id_paciente', $comida->id_paciente) == $paciente->id_paciente ? 'selected' : '' }}>
                        {{ $paciente->usuario->nombre ?? 'Paciente #' . $paciente->id_paciente }}
                      </option>
                    @endforeach
                  @endisset
                </select>
              ...
              <div class="col-md-6">
                ...
                <select class="form-select form-control-custom @error('tipo_registro') is-invalid @enderror" 
                  ...
                  <option value="MANUAL" {{ old('tipo_registro', $comida->tipo_registro) == 'MANUAL' ? 'selected' : '' }}>Manual</option>
                  <option value="FOTO" {{ old('tipo_registro', $comida->tipo_registro) == 'FOTO' ? 'selected' : '' }}>Foto</option>
                </select>
                @error('tipo_registro')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              ...
              <div class="col-md-6">
                ...
                <input type="datetime-local" class="form-control form-control-custom @error('fecha_hora') is-invalid @enderror" 
                       id="fecha_hora" name="fecha_hora" value="{{ old('fecha_hora', $comida->fecha_hora ? $comida->fecha_hora->format('Y-m-d\TH:i') : '') }}">
                @error('fecha_hora')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="col-md-12 mt-4">
                <div class="d-flex gap-2">
                  <button type="submit" class="btn btn-success-custom">
                    <i class="bi bi-check-circle me-2"></i>Actualizar Comida
                  </button>
