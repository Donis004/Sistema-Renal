...
<head>
  <meta charset="UTF-8" />
  <title>Editar Alergia - Sistema Renal</title>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css" rel="stylesheet">
  <style>
    body { background-color: #c9f3d6; font-family: 'Segoe UI', sans-serif; }
    .app-wrapper { background: #f6fff9; border-radius: 20px; padding: 25px; margin: 30px; box-shadow: 0 15px 40px rgba(0,0,0,.1); }
    ...
    .card-custom { background: white; border-radius: 15px; padding: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); }
    .info-box { background: #e0f2fe; border-left: 4px solid #1fbf83; padding: 15px; margin-bottom: 20px; border-radius: 8px; }
    ...
    .btn-action { background: linear-gradient(135deg, #1fbf83, #38b2ac); color: white; padding: 12px 30px; border-radius: 12px; border: none; font-weight: 600; }
    ...
    <div class="row g-4">
      ...
      <div class="col-md-10">
        <div class="header-section">
          <h2><i class="bi bi-pencil me-2"></i>Editar Alergia</h2>
          <p class="mb-0 opacity-75">Actualiza los datos de la alergia</p>
        </div>
        <div class="card-custom">
          <div class="info-box">
            <strong>ID:</strong> #{{ $alergia->id_alergia }}
          </div>
          <form action="{{ route('administrador.alergias.update', $alergia->id_alergia) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
              ...
              <input type="text" name="nombre" id="nombre" class="form-control" value="{{ old('nombre', $alergia->nombre) }}" required>
            </div>
            <div class="d-flex gap-3">
              <button type="submit" class="btn btn-action">
                <i class="bi bi-check-circle me-2"></i>Actualizar
              </button>
              <a href="{{ route('administrador.alergias.show', $alergia->id_alergia) }}" class="btn btn-cancel">
