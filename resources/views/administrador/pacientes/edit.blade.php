...
<head>
  <meta charset="UTF-8" />
  <title>Editar Paciente - Sistema Renal</title>
      ...
      <div class="col-md-10">
        ...
        <div class="header-section">
          <div class="row align-items-center">
            <div class="col-md-8">
              <h2 class="mb-1"><i class="bi bi-pencil-square me-2"></i>Editar Paciente</h2>
              <p class="mb-0 opacity-75">Modifica los datos del paciente</p>
            </div>
        ...
        <div class="card-custom">
          <form method="POST" action="{{ route('administrador.pacientes.update', $paciente->id_paciente) }}">
            @csrf
            @method('PUT')
              ...
              <div class="col-md-12">
                ...
                <select class="form-select form-control-custom @error('id_usuario') is-invalid @enderror" 
                  ...
                  @foreach($usuarios as $usuario)
                    <option value="{{ $usuario->id_usuario }}" {{ old('id_usuario', $paciente->id_usuario) == $usuario->id_usuario ? 'selected' : '' }}>
            ...
            <div class="row g-3 mb-4">
              <div class="col-md-4">
                ...
                <input type="date" class="form-control form-control-custom @error('fecha_nacimiento') is-invalid @enderror" 
                       id="fecha_nacimiento" name="fecha_nacimiento" value="{{ old('fecha_nacimiento', $paciente->fecha_nacimiento) }}">
              ...
              <div class="col-md-4">
                ...
                <input type="number" step="0.01" class="form-control form-control-custom @error('peso_kg') is-invalid @enderror" 
                       id="peso_kg" name="peso_kg" value="{{ old('peso_kg', $paciente->peso_kg) }}" placeholder="Ej: 75.50">
                @error('peso_kg')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="col-md-6">
                ...
                <input type="text" class="form-control form-control-custom @error('presion_arterial') is-invalid @enderror" 
                       id="presion_arterial" name="presion_arterial" value="{{ old('presion_arterial', $paciente->presion_arterial) }}" placeholder="Ej: 120/80">
              ...
              <div class="col-md-6">
                ...
                <input type="number" step="0.01" class="form-control form-control-custom @error('egfr') is-invalid @enderror" 
                       id="egfr" name="egfr" value="{{ old('egfr', $paciente->egfr) }}" placeholder="Ej: 45.50">
                @error('egfr')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="col-md-12">
                ...
                <select class="form-select form-control-custom @error('etapa_erc') is-invalid @enderror" 
                  ...
                  <option value="1" {{ old('etapa_erc', $paciente->etapa_erc) == '1' ? 'selected' : '' }}>Etapa 1 (EGFR ≥ 90)</option>
                  <option value="2" {{ old('etapa_erc', $paciente->etapa_erc) == '2' ? 'selected' : '' }}>Etapa 2 (EGFR 60-89)</option>
                  <option value="3a" {{ old('etapa_erc', $paciente->etapa_erc) == '3a' ? 'selected' : '' }}>Etapa 3a (EGFR 45-59)</option>
                  <option value="3b" {{ old('etapa_erc', $paciente->etapa_erc) == '3b' ? 'selected' : '' }}>Etapa 3b (EGFR 30-44)</option>
                  <option value="4" {{ old('etapa_erc', $paciente->etapa_erc) == '4' ? 'selected' : '' }}>Etapa 4 (EGFR 15-29)</option>
                  <option value="5" {{ old('etapa_erc', $paciente->etapa_erc) == '5' ? 'selected' : '' }}>Etapa 5 (EGFR < 15)</option>
                </select>
                @error('etapa_erc')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="row g-3 mb-4">
              <div class="col-md-12">
                ...
                <textarea class="form-control form-control-custom @error('dieta_prescrita') is-invalid @enderror" 
                          id="dieta_prescrita" name="dieta_prescrita" rows="4" placeholder="Describe el plan dietético para el paciente...">{{ old('dieta_prescrita', $paciente->dieta_prescrita) }}</textarea>
            ...
            <div class="col-md-12 mt-4 mb-4">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" id="perfil_completo" name="perfil_completo" value="1" 
                       {{ old('perfil_completo', $paciente->perfil_completo) ? 'checked' : '' }}>
