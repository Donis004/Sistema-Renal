...
<html lang="es">
<head>
  <meta charset="UTF-8" /><title>Síntomas - Sistema Renal</title>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css" rel="stylesheet">
  <style>
    :root { --primary-green: #1fbf83; }
    ...
    .sidebar { background: #ffffff; border-radius: 20px; padding: 20px; }
    ...
    .table-custom tbody tr { border-bottom: 1px solid #e5e7eb; }
      ...
      <div class="col-md-10">
        <div class="header-section">
          <div class="row align-items-center">
            <div class="col-md-8">
              <h2 class="mb-2"><i class="bi bi-thermometer me-2"></i>Síntomas</h2>
              <p class="mb-0 opacity-75">Gestiona los síntomas que reportan los pacientes</p>
            </div>
            <div class="col-md-4 text-end">
              <a href="{{ route('administrador.sintomas.create') }}" class="btn btn-create"><i class="bi bi-plus-circle me-2"></i>Nuevo Síntoma</a>
            </div>
          </div>
        </div>
        @if(session('success'))
        ...
        <div class="card-custom">
          @if($sintomas->count() > 0)
            <div style="overflow-x: auto;">
              <table class="table table-custom">
                ...
                <tbody>
                  ...
                  <tr>
                    <td><strong>{{ $sintoma->nombre }}</strong></td>
                    <td><span class="badge-count">{{ $sintoma->registroSintomas->count() }}</span></td>
                    <td>
                      <a href="{{ route('administrador.sintomas.show', $sintoma->id_sintoma) }}" class="btn btn-action btn-view"><i class="bi bi-eye me-1"></i>Ver</a>
                      <a href="{{ route('administrador.sintomas.edit', $sintoma->id_sintoma) }}" class="btn btn-action btn-edit"><i class="bi bi-pencil me-1"></i>Editar</a>
                      <form action="{{ route('administrador.sintomas.destroy', $sintoma->id_sintoma) }}" method="POST" class="d-inline">
                        @csrf @method('DELETE')
                        <button type="submit" class="btn btn-action btn-delete" onclick="return confirm('¿Seguro?')"><i class="bi bi-trash me-1"></i>Eliminar</button>
                      </form>
                    </td>
                  </tr>
                  @endforeach
            ...
            <div class="d-flex justify-content-center mt-4 mb-3">{{ $sintomas->links() }}</div>
          @else
            <div class="empty-state"><i class="bi bi-inbox"></i><p>No hay síntomas</p>
              <a href="{{ route('administrador.sintomas.create') }}" class="btn btn-create mt-3"><i class="bi bi-plus-circle me-2"></i>Crear</a>
            </div>
          @endif
        </div>
      </div>
